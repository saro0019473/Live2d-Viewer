...
export class Live2DCoreManager {
  ...
  public app: any;
  public modelContainer: any;
  public isInitialized: boolean;
  public logger: Logger;

  // WebGL context loss handling state
  private _contextLost: boolean;
  private _contextLossCount: number;
  private _maxContextRestoreAttempts: number;
  private _contextRestoreTimeoutId: number | null | NodeJS.Timeout;
  private _boundHandleContextLost: (event: any) => void;
  private _boundHandleContextRestored: (event: any) => void;

  // Callbacks that external code (e.g. Live2DManager) can register
  public onContextLost: (() => void) | null;
  public onContextRestored: (() => void) | null;
   ...
   * @private
   */
  private _unregisterContextLossHandlers(): void {
    ...
    if (this._contextRestoreTimeoutId !== null) {
      clearTimeout(this._contextRestoreTimeoutId as any);
      this._contextRestoreTimeoutId = null;
    }
  }
   ...
   * @private
   */
  private _handleContextLost(event: WebGLContextEvent): void {
   ...
   * @private
   */
  private _handleContextRestored(_event: Event): void {
    this._contextLost = false;
  ...
  optimizePerformance(options: {
    ...
    textureGCMode?: "aggressive" | "conservative" | "auto";
  } = {}): void {
    if (!this.app) return;
  ...
  destroy(): void {
    ...
    const canvas = this.app?.view;
    const suppressHandler = (e: Event) => e.preventDefault();
    if (canvas) {
