/**
 * HeroModel — Per-Model Wrapper
 *
 * Wraps a single PIXI `Live2DModel` instance and adds:
 *   - Structured access to expressions, motions, and parameters
 *   - Safe setter utilities (ParameterUtils) that also keep internal state in sync
 *   - Model metadata (name, costume, version info)
 *
 * There is exactly ONE HeroModel per loaded model.
 * The raw PIXI model is in `this.model` — use it only for stage/transform operations.
 * For parameter/expression/motion control, use HeroModel's own methods.
 *
 * NOTE: `model` is typed as `any` because PIXI-Live2D-Display does not ship TypeScript
 * declarations. Add a comment whenever you use `model` directly so AI assistants
 * can understand the intent.
 *
 * Lifecycle:
 *   Created by ModelManager.load() → registered with StateSyncManager
 *   Destroyed by ModelManager.removeModel() → calls this.destroy()
 */

import { waitForLive2D, createLogger, type Logger } from "./utils";
  ...
  setParameterValue(
    ...
    try {
      ...
      const paramIndex = parametersValues.parameter?.findIndex(
        (param: any) => param.parameterIds === paramId,
      );
      if (paramIndex !== -1 && parametersValues.parameter) {
...
export class HeroModel {
  ...
  public cubismModelSettings: any;
  public rawModelSettings: any;
  public cachedExpressions: any[];
  public cachedMotions: Record<string, any[]>;
  ...
  /**
   * Helper to perform unified model validity checks
   */
  private checkModel(operationName: string = "operation"): boolean {
    if (!this.model) {
   ...
   */
  async create(src: string): Promise<HeroModel> {
    try {
      ...
      if (this.model.internalModel) {
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          this.model.once("error", (error: any) => {
            clearTimeout(timeout);
            reject(error);
          });
        });
  ...
  initializeParameters(retries: number = 5, delay: number = 200): void {
    if (this._destroyed) {
      this.logger.log(
   ...
   */
  setName(char: string, cost: string): void {
    this.modelName = char;
    this.costume = cost;
  }

  public setAnchor(x: number, y: number): boolean | void {
    if (!this.checkModel("set anchor")) return false;
  ...
  public setScale(val: number): boolean | void {
    if (!this.checkModel("set scale")) return false;
  ...
  public setVisible(val: boolean): boolean | void {
    if (!this.checkModel("set visibility")) return false;
  ...
  public setAngle(val: number): boolean | void {
    if (!this.checkModel("set angle")) return false;
  ...
  public setAlpha(val: number): boolean | void {
    if (!this.checkModel("set alpha")) return false;
  ...
  public setPosition(x: number, y: number): boolean | void {
    if (!this.checkModel("set position")) return false;
   ...
   */
  public setForeground(sprite: any): void {
   ...
   */
  public setBreathing(bool: boolean): boolean | void {
    if (!this.checkModel("set breathing")) return false;
   ...
   */
  public setEyeBlinking(bool: boolean): boolean | void {
    if (!this.checkModel("set eye blinking")) return false;
   ...
   */
  public setInteractive(bool: boolean): boolean | void {
    if (!this.checkModel("set interactivity")) return false;
   ...
   */
  public setLookatMouse(bool: boolean): boolean | void {
    if (!this.checkModel("set gaze tracking")) return false;
   ...
   */
  public setExpression(index: number): boolean {
    if (!this.checkModel("play expression")) return false;
    try {
      if (!this.model.internalModel) {
   ...
   * @returns {boolean} Whether playback was successful
   */
  public async playMotion(group: string, index: number): Promise<boolean> {
    try {
      if (!this.checkModel("play motion")) return false;
      if (!this.model.internalModel) {
        this.logger.warn("⚠️ Internal model not ready");
        return false;
      }
      ...
      this.logger.error("❌ play motion failed:", error);
      return false;
    }
  }
    ...
    try {
      ...
      if (state.partOpacity && this.model?.internalModel?.coreModel) {
        Object.entries(state.partOpacity).forEach(([partId, value]) => {
          this.model.internalModel.coreModel.setPartOpacityById(partId, value as number);
        });
      }
   ...
   * @param {number} index - Motion index
   */
  scheduleStateRestore(initialState: any, group: string, index: number): void {
   ...
   */
  public async playRandomMotion(group: string | null = null): Promise<boolean> {
    if (!this.checkModel("play random motion")) return false;
  ...
  stopAllMotions(): void {
    ...
    // Clear _isLoop flag from all cached motions to prevent lingering loops
    // and stuttering when switching motions or replaying a motion.
    if (motionManager.motionGroups) {
      for (const groupName in motionManager.motionGroups) {
        const group = motionManager.motionGroups[groupName];
        if (Array.isArray(group)) {
          for (let i = 0; i < group.length; i++) {
            const motionObj = group[i];
   ...
   */
  getCurrentExpressionIndex(): number | null {
    if (
      !this.model ||
      !this.model.internalModel ||
   ...
   */
  getMotionAudioUrl(group: string, index: number): string | null {
    const motionGroup = this.cachedMotions[group];
   ...
   */
  setParameters(paramId: string, value: number, weight: number = 1): boolean {
   ...
   */
  setPartOpacity(partId: string, value: number): boolean {
   ...
   */
  public setForegroundVisible(visible: boolean): boolean | void {
    if (!this.checkModel("set foreground visibility")) return false;
    if (this.foreground) {
   ...
   */
  setModelProperties(modelData: any): void {
   ...
   */
  public autoFitToCanvas(
    canvasWidth: number,
    canvasHeight: number,
    targetHeightRatio: number = 0.5,
  ): boolean {
    if (!this.checkModel("fit to canvas")) return false;
    try {
   ...
   */
  getModelOriginalSize(): { width: number; height: number } | null {
    if (!this.model) {
      return null;
    }
   ...
   * @param {number} canvasHeight - Canvas height
   */
  public resetToDefault(canvasWidth?: number, canvasHeight?: number): boolean {
    if (!this.checkModel("reset model")) return false;
    try {
   ...
   */
  public forceDefaultScale(defaultScale: number = 0.2): boolean {
    if (!this.checkModel("set default scale")) return false;
    try {
