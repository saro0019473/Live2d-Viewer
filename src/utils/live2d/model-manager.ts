...
export class Live2DModelManager {
  public coreManager: Live2DCoreManager & { interactionManager?: any };
  public models: Map<string, HeroModel>;
  public currentModelId: string | null;
  public isPetMode: boolean;
  public petModeScale: number;
  public logger: Logger;

  constructor(coreManager: Live2DCoreManager & { interactionManager?: any }) {
    this.coreManager = coreManager;
    this.models = new Map<string, HeroModel>();
    ...
    if (enabled) {
      ...
      this.models.forEach((model) => {
        if (model.model) {
          ...
          model.setAnchor(0.5, 0.5); // Ensure anchor is correct
        }
      });
   ...
   */
  public setPetModeScale(scale: number): void {
   ...
   */
  public async loadModel(modelData: ModelData): Promise<HeroModel> {
    ...
    try {
      ...
      heroModel = new HeroModel(modelId, undefined);
   ...
   * @param {ModelData} modelData - Model data
   */
  public async refreshModel(modelId: string, modelData: ModelData): Promise<HeroModel> {
    try {
   ...
   * @param {string} modelId - Model ID
   */
  public addModelToScene(heroModel: HeroModel, modelId: string): void {
   ...
   */
  public calculateModelPosition(): { x: number; y: number } {
   ...
   * @private
   */
  private _applyModelLayout(heroModel: HeroModel): void {
    // Set model properties
    heroModel.setAnchor(0.5, 0.5);
   ...
   * Get model
   */
  public getModel(modelId: string): HeroModel | undefined {
    return this.models.get(modelId);
  }
   ...
   */
  public getModelId(heroModel: HeroModel): string | null {
   ...
   */
  public getCurrentModel(): HeroModel | null {
   ...
   */
  public getFirstAvailableModel(): HeroModel | null {
    const modelIds = Array.from(this.models.keys());
    if (modelIds.length === 0) return null;
    return this.models.get(modelIds[0]) || null;
  }
