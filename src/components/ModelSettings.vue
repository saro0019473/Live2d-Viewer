    ...
    <div class="view-container">
        <n-spin :show="!currentModel || loading">
            <div v-if="currentModel">
                <!-- Hero Model Header -->
                <div class="model-settings-hero">
                    <div class="model-meta-info">
                        <div class="model-hero-name">
                            {{
                                getModelDisplayName(
                                    (currentModel as any)?.url,
                                ) || "No Model Selected"
                            }}
                        </div>
                        <div class="model-hero-url">
                            {{ (currentModel as any)?.url || "" }}
                        </div>
                    </div>
                    <n-tag
                ...
                <n-collapse>
                    <!-- Display Settings -->
                    <n-collapse-item title="Display Settings" name="display">
                        <template #header-extra>
                            <n-space>
                                <n-tag size="small" type="primary">Basic</n-tag>
                                <n-icon
                                    size="16"
                        ...
                        >
                            <n-space vertical size="medium">
                                ...
                                <setting-slider
                                    ...
                                    :updater="
                                        (val) => currentHeroModel?.setScale(val)
                                    "
                                    @update:model-value="
                                ...
                                <setting-slider
                                    ...
                                    :show-min-max-input="false"
                                    :updater="
                                        (val) => currentHeroModel?.setAngle(val)
                                    "
                                    @update:model-value="
                                        (val) => (modelSettings.rotation = val)
                                    "
                                />

                                <n-divider
                                    style="margin: var(--app-gap-sm) 0"
                                />
                                ...
                                <n-space vertical size="medium">
                                    ...
                                    <setting-switch
                                        label="Mouse Follow"
                                        :model-value="modelSettings.lookAtMouse"
                                        @update:model-value="
                                            (value: boolean) => {
                    ...
                    >
                        <template #header-extra>
                            <n-space>
                                ...
                                <n-button
                                    size="tiny"
                                    :type="motionLoop ? 'warning' : 'default'"
...
/**
 * ModelSettings.vue — Right Panel: Model Parameter Controls
 *
 * Organized into collapsible sections (n-collapse-item):
 *   1. Display Settings — scale, position, rotation (sliders via SettingSlider)
 *   2. Behavior Settings — breathing, eye blinking, click interaction toggles
 *   3. Expression Panel  — lists model expressions; click to trigger
 *   4. Motion Panel      — lists motion groups/indices; play / loop controls
 *   5. Parameter Tuning  — raw Cubism parameter sliders (advanced)
 *
 * Settings sync:
 *   - Reads `live2dStore.modelState.settings` for display
 *   - Writes back via `live2dStore.updateSetting(key, value)` (debounced)
 *   - Uses `globalStateSyncManager` for real-time expression/parameter state
 *   - `settingsSyncEnabled` flag prevents feedback loops during batch updates
 *
 * Emits: `back` — signals App.vue to close the settings panel
 */
import {
    ref,
    reactive,
...
const syncDebounceTimer = ref<ReturnType<typeof setTimeout> | null>(null);
...
const currentParameters = reactive<Record<string, number>>({});
const currentParts = reactive<Record<string, number>>({});
...
const motions = computed(() => {
    ...
    try {
        ...
        console.error("❌ [ModelSettings] Failed to get motion data:", error);
        return {};
    }
});
...
const partOpacity = computed(() => {
    ...
    try {
        ...
        console.error("❌ [ModelSettings] Failed to get part data:", error);
        return [];
    }
});
...
const cleanupResources = () => {
    ...
    __DEV__ && console.debug("[ModelSettings] Resource cleanup completed");
};
...
const loadSettingsFromStore = () => {
    ...
    try {
        ...
        // Extended state
        applySetting("expression", currentExpression, settings as any, null);
        applySetting("motion", currentPlayingMotion, settings as any, null);
        if ((settings as any).parameters !== undefined) {
        ...
        if ((settings as any).parts !== undefined) {
            // Clear existing parts
            for (const key in currentParts) {
                delete currentParts[key];
            }
            // Copy new parts
            Object.assign(currentParts, (settings as any).parameters);
        }
        applySetting("audio", currentAudioState, settings as any, false);
        applySetting("text", currentTextState, settings as any, false);

        __DEV__ && console.debug("[ModelSettings] Settings loaded from Store");
    } finally {
        isLoadingFromStore.value = false;
    }
};
...
const syncAllModelStatesToUI = () => {
    ...
    try {
        ...
        if (currentHeroModel.value.model) {
            ...
            const interactionMgr = (live2dStore?.manager as any)
                ?.interactionManager as
                | { isDragEnabled?: boolean; lookAtMouseEnabled?: boolean }
                | undefined;
            if (interactionMgr !== undefined) {
                modelSettings.interactive =
                    interactionMgr.isDragEnabled !== undefined
                        ? interactionMgr.isDragEnabled
                        : (currentHeroModel.value as any).model.interactive;
            } else {
                modelSettings.interactive = (
                    currentHeroModel.value as any
                ).model.interactive;
            }
            ...
            if (interactionMgr !== undefined) {
                ...
                modelSettings.lookAtMouse =
                    (currentHeroModel.value as any).model.focusing === true;
            }
        }
        ...
        if (motionManager && motionManager.currentMotion) {
            ...
            currentPlayingMotion.value = {
                group: (currentMotion as any).group,
                index: (currentMotion as any).index,
                isPlaying: true,
                name:
                    (currentMotion as any).name ||
                    `${(currentMotion as any).group}_${(currentMotion as any).index}`,
            } as any;
        ...
        __DEV__ && console.debug("[ModelSettings] Full state sync completed.");
    } catch (error) {
        console.error("❌ [ModelSettings] Failed to sync model state:", error);
    ...
    async (newModel, oldModel) => {
        if (newModel && newModel.id !== oldModel?.id) {
            ...
            if (newModel && currentHeroModel.value) {
                const restored = globalStateSyncManager.restoreStateFromCache(
                    newModel.id,
...
watch(currentExpression, (newExpression) => {
    if (newExpression !== null) {
        __DEV__ &&
            console.debug(`[ModelSettings] Expression set: ${newExpression}`);
        syncSettingsToStore();
    }
});
...
watch([currentAudioState, currentTextState], () => {
    __DEV__ && console.debug("[ModelSettings] Audio/text state updated");
    syncSettingsToStore();
});
...
const playMotion = (group: string, index: number, motion: any) => {
    if (!currentHeroModel.value) return;
    ...
    message.success(
        `Playing motion: ${(currentPlayingMotion.value as any)?.name ?? `${group}_${index}`}`,
    );
    syncSettingsToStore();
};
...
let currentMotionAudio: HTMLAudioElement | null = null;
let currentMotionAudioBlobUrl: string | null = null;

const playMotionAudio = async (group: string, index: number, motion: any) => {
    try {
        if (!currentHeroModel.value) return;
        ...
        if (isCrossOrigin) {
            ...
            if (!modelSettings.enableAudio || !currentMotionAudioBlobUrl) {
        ...
        currentMotionAudio.addEventListener("error", (e) => {
            console.warn("⚠️ Motion audio playback error:", audioUrl, e);
            if (currentMotionAudioBlobUrl) {
                URL.revokeObjectURL(currentMotionAudioBlobUrl);
                currentMotionAudioBlobUrl = null;
            }
        ...
        currentMotionAudio.play().catch((err) => {
            console.warn("⚠️ Motion audio play() rejected:", err.message);
            if (currentMotionAudioBlobUrl) {
                URL.revokeObjectURL(currentMotionAudioBlobUrl);
                currentMotionAudioBlobUrl = null;
            }
...
const playRandomMotion = () => {
    if (!currentHeroModel.value || Object.keys(motions.value).length === 0)
        return;
...
const playRandomMotionFromGroup = (groupName: string) => {
...
const isCurrentMotion = (group: string, index: number) => {
    return (
        currentPlayingMotion.value &&
        (currentPlayingMotion.value as any).group === group &&
        (currentPlayingMotion.value as any).index === index
    );
};
...
const updateClickInteraction = () => {
    try {
        ...
        if (live2dStore?.manager?.interactionManager) {
            ...
            if (typeof live2dStore.manager.setDragEnabled === "function") {
                live2dStore.manager.setDragEnabled(modelSettings.interactive);
            }
        }
...
const registerStateSync = () => {
    ...
    globalStateSyncManager.registerSyncCallback(modelId, (currentState) => {
        ...
        const applyStateToUI = (
            ...
            if (sourceValue !== undefined) {
                if (
                    ...
                    Math.abs(sourceValue - targetRef.value) > tolerance
                ) {
                ...
                if (
                    (modelSettings as any)[key] !== undefined &&
                    typeof (modelSettings as any)[key] === "boolean"
                ) {
                    (modelSettings as any)[key] = sourceValue;
                }
            }
        };
        ...
        // Complex object sync
        if (
            (currentState as any).expression !== undefined &&
            (currentState as any).expression !== currentExpression.value
        ) {
            currentExpression.value = (currentState as any).expression;
        }

        if ((currentState as any).motion !== undefined) {
            if (
                (currentState as any).motion &&
                JSON.stringify((currentState as any).motion) !==
                    JSON.stringify(currentPlayingMotion.value)
            ) {
                currentPlayingMotion.value = (currentState as any).motion;
                isMotionPlaying.value = (
                    (currentState as any).motion as any
                ).isPlaying;
            } else if (!(currentState as any).motion && isMotionPlaying.value) {
                isMotionPlaying.value = false;
                currentPlayingMotion.value = null;
            }
        }
    ...
    __DEV__ &&
        console.debug(`[ModelSettings] State sync registered: ${modelId}`);
};
...
const unregisterStateSync = () => {
    ...
    __DEV__ &&
        console.debug(`[ModelSettings] State sync unregistered: ${modelId}`);
};
...
const getModelDisplayName = (url: any) => {
...
const getMotionDisplayName = (motion: any, index: number) => {
